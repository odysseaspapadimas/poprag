 ...
 */
export async function testVectorizeQuery(namespace: string, sampleQuery: string) {
	try {
		const { env } = await import("cloudflare:workers");
		...
		console.log("[Vectorize Test Query]", {
			...
			metadataKeys: results.matches?.[0]?.metadata ? Object.keys(results.matches[0].metadata) : [],
		});
